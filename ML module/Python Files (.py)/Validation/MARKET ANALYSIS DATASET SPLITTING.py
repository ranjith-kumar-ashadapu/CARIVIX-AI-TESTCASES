# ==========================================
# MARKET ANALYSIS DATASET SPLITTING
# TRAINING, VALIDATION, AND TESTING
# ==========================================

import os
import hashlib
import pandas as pd

from sklearn.model_selection import train_test_split

# ==========================================
# FILE PATHS
# ==========================================

input_file = (
    r"D:\OneDrive\Desktop\Day 3 Task"
    r"\Market_Analysis_Feature_Engineering"
    r"\Market_Analysis_Feature_Engineered.csv"
)

output_folder = (
    r"D:\OneDrive\Desktop\Day 3 Task"
    r"\Market_Analysis_Data_Splits"
)

os.makedirs(output_folder, exist_ok=True)

train_file = os.path.join(
    output_folder,
    "Market_Analysis_Train.csv"
)

validation_file = os.path.join(
    output_folder,
    "Market_Analysis_Validation.csv"
)

test_file = os.path.join(
    output_folder,
    "Market_Analysis_Test.csv"
)

# ==========================================
# LOAD DATASET
# ==========================================

df = pd.read_csv(input_file)

print("=" * 60)
print("MARKET ANALYSIS DATASET SPLITTING")
print("=" * 60)

print("Original Dataset Shape:", df.shape)

# ==========================================
# REMOVE DUPLICATES
# ==========================================

duplicate_count = df.duplicated().sum()

print("Duplicate Rows:", duplicate_count)

df = df.drop_duplicates().reset_index(drop=True)

print("Dataset Shape After Removing Duplicates:", df.shape)

# ==========================================
# CREATE UNIQUE ROW IDENTIFIERS
# ==========================================

def create_row_hash(row):
    row_text = "||".join(
        row.astype(str).tolist()
    )
    return hashlib.md5(
        row_text.encode("utf-8")
    ).hexdigest()

row_hashes = df.apply(
    create_row_hash,
    axis=1
)

# Confirm that row identifiers are unique
if row_hashes.duplicated().any():

    raise ValueError(
        "Duplicate records were found after row-hash creation."
    )

# ==========================================
# SPLIT DATASET
# ==========================================

random_seed = 42

# First split:
# 70% training
# 30% temporary data

train_df, temporary_df = train_test_split(
    df,
    test_size=0.30,
    random_state=random_seed,
    shuffle=True
)

# Second split:
# Temporary data is divided equally:
# 15% validation
# 15% testing

validation_df, test_df = train_test_split(
    temporary_df,
    test_size=0.50,
    random_state=random_seed,
    shuffle=True
)

# Reset indexes
train_df = train_df.reset_index(drop=True)
validation_df = validation_df.reset_index(drop=True)
test_df = test_df.reset_index(drop=True)

# ==========================================
# VERIFY DATASET SIZES
# ==========================================

total_records = len(df)

print("\nDataset Split Summary")
print("-" * 60)

print(
    "Training Records:",
    len(train_df),
    f"({len(train_df) / total_records * 100:.2f}%)"
)

print(
    "Validation Records:",
    len(validation_df),
    f"({len(validation_df) / total_records * 100:.2f}%)"
)

print(
    "Testing Records:",
    len(test_df),
    f"({len(test_df) / total_records * 100:.2f}%)"
)

print(
    "Total Split Records:",
    len(train_df) + len(validation_df) + len(test_df)
)

# ==========================================
# VERIFY NO OVERLAPPING RECORDS
# ==========================================

train_hashes = set(
    train_df.apply(create_row_hash, axis=1)
)

validation_hashes = set(
    validation_df.apply(create_row_hash, axis=1)
)

test_hashes = set(
    test_df.apply(create_row_hash, axis=1)
)

train_validation_overlap = (
    train_hashes.intersection(validation_hashes)
)

train_test_overlap = (
    train_hashes.intersection(test_hashes)
)

validation_test_overlap = (
    validation_hashes.intersection(test_hashes)
)

print("\nOverlap Verification")
print("-" * 60)

print(
    "Training-Validation Overlap:",
    len(train_validation_overlap)
)

print(
    "Training-Testing Overlap:",
    len(train_test_overlap)
)

print(
    "Validation-Testing Overlap:",
    len(validation_test_overlap)
)

if (
    len(train_validation_overlap) > 0
    or len(train_test_overlap) > 0
    or len(validation_test_overlap) > 0
):

    raise ValueError(
        "Overlapping records were found between dataset splits."
    )

print("No duplicate records found across the splits.")

# ==========================================
# VERIFY COLUMN CONSISTENCY
# ==========================================

if not (
    list(train_df.columns)
    == list(validation_df.columns)
    == list(test_df.columns)
):

    raise ValueError(
        "The columns are not consistent across the splits."
    )

print("Column consistency verified.")

# ==========================================
# SAVE DATASETS
# ==========================================

train_df.to_csv(
    train_file,
    index=False
)

validation_df.to_csv(
    validation_file,
    index=False
)

test_df.to_csv(
    test_file,
    index=False
)

# ==========================================
# FINAL SUMMARY
# ==========================================

print("\n" + "=" * 60)
print("DATASET SPLITTING COMPLETED SUCCESSFULLY")
print("=" * 60)

print("Training Dataset:")
print(train_file)

print("\nValidation Dataset:")
print(validation_file)

print("\nTesting Dataset:")
print(test_file)

print("=" * 60)
